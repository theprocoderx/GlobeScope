...
const Footer = () => {
  ...
  return (
    <footer
      className={`mx-auto flex w-full max-w-7xl flex-col items-center px-6 py-8 transition-colors duration-300 ${
        isDark ? 'bg-[#050816] text-[#b0b2c3]' : 'border-t border-slate-200 bg-slate-200 text-slate-600'
      }`}
    >
      {/* Logo */}
      <div className='mb-6'>
      ...
      {/* CTA */}
      <p className={`mb-5 text-center text-sm ${isDark ? 'text-slate-300' : 'text-slate-700'}`}>
      ...
      {/* Divider */}
      <div className={`my-6 h-px w-full ${isDark ? 'bg-white/10' : 'bg-slate-300'}`} />
